type ProjectOverviewProp = {
  title?: string;
  totalProjects?: number;
};

const ProjectOverview = ({ title, totalProjects }: ProjectOverviewProp) => {
  return (
    <section className='w-full flex flex-col gap-4 items-start justify-start py-12 px-10 bg-black h-68'>
      {/* Title */}
      <div className='w-full flex items-start justify-start flex-col gap-5'>
        <h2 className='text-4xl font-bold text-white'>{title || "Projects"}</h2>
      </div>
      {/* Info */}
